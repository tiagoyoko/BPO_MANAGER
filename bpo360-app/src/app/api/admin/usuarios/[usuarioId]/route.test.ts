...
describe("PATCH /api/admin/usuarios/[usuarioId]", () => {
  it("retorna 400 quando clienteId não pertence ao mesmo BPO", async () => {
    ...
    const req = new Request("http://localhost/api/admin/usuarios/u1", {
      ...
      body: JSON.stringify({ clienteId: "cliente-outro-bpo" }),
    });
  ...
  it("atualiza usuário quando clienteId pertence ao mesmo BPO", async () => {
    getCurrentUser.mockResolvedValue(ADMIN);
    canManageUsers.mockReturnValue(true);
    createClient.mockResolvedValue(
      makeSupabaseMock() as unknown as Awaited<ReturnType<typeof createClient>>
    ...
    const req = new Request("http://localhost/api/admin/usuarios/u1", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ nome: "Nome Atualizado", clienteId: "cliente-1" }),
    });
